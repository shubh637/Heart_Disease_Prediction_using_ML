    ...
    if history:
      ...
      # Add a button to clear history
        if st.button("Clear History"):
            ...
            st.experimental_rerun()  # Reloads the app to reflect changes after clearing history
    else:
...
def start():
    ...
    with st.sidebar:
        selected=option_menu("Multiple Disease Prediction System using ML",
                             ["Home","Disease Predictor","Diabetes Prediction",
                              ...
                              "Cardiovascular Disease Prediction","Penumonia Prediction","History",
                              "LogOut"],
                              icons=["house","capsule","activity","heart","person","lungs","book","box-arrow-right"],
                              default_index=0)
    ...
    elif st.session_state.selected=="Penumonia Prediction":
...
def pneumonia_prediction_page():
    ...
    components.html(penumonia_page_image,height=500)
# Streamlit application
    st.title('Pneumonia Detection from Chest X-ray Images')
    st.write('Upload a chest X-ray image to predict if it is normal or indicates pneumonia.')
...
# File uploader
    ...
    # Predict using the pre-trained model
        ...
        st.session_state.pneumonia_prediction = prediction[0][0]
        result="Positive" if prediction[0][0]>0.5 else "Negative"
        if 'username' not in st.session_state or not st.session_state.username:
            st.error("Please Log In")
            st.session_state.page="Login"
        else: 
            save_test_history(st.session_state.username, "Pneumonia Disease", datetime.now().strftime("%Y-%m-%d %H:%M:%S"),result)
        st.session_state.page = 'pneumonia_result'
        st.experimental_rerun()
...
def framingham_result_page():
    ...
    if st.button("Back to Home"):
        st.session_state.page = 'start'
        st.experimental_rerun()
def pneumonia_result_page():
    if st.session_state.pneumonia_prediction > 0.5:
        st.error('The person is likely to have Penumonia.')
        components.html(penumonia_result_image,height=470)
        components.html(hover+penumonia_result_if,height=670)
            
    else:
        st.success('The person is not likely to have Penumonia.')
        components.html(hover+no_disease_image,height=470)
        components.html(hover+penumonia_page_else,height=670)
    if st.button("Back to Home"):
        st.session_state.page = 'start'
        st.experimental_rerun()    
...
elif st.session_state.page=="pneumonia_result":
    pneumonia_result_page()    
